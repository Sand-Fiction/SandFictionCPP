...
void USF_InteractableComponent::BeginPlay()
{
	...
	{
		InteractWidgetComponent = NewObject<USF_InteractionWidgetComponent>(
			GetOwner(), USF_InteractionWidgetComponent::StaticClass(), FName("InteractWidgetComponent"));

		InteractWidgetComponent->SetupAttachment(GetOwner()->GetRootComponent());
		InteractWidgetComponent->RegisterComponent();

		GetOwner()->AddInstanceComponent(InteractWidgetComponent);
